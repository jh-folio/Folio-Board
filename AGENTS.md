...
- **테스트는 실제 시장 캐시에 쓰지 못한다.** heatmap fetcher를 스텁하지 않은 테스트가 `collect_briefing_visuals`를 불러 실제 yfinance를 때리고 요청일(2026-07-31) 스냅샷을 **실제 last-good 캐시에 저장**했다 — provider가 지연된 날(아시아 일봉은 마감 후 반나절 넘게 히스토리 API에 안 실리기도 한다. 실측: 마감 12시간 뒤에도 08-27 봉 부재) fallback한 브리핑 두 건이 한 달 전 히트맵을 실었고 삼성전자 +26.8% 같은 비일일 등락이 그대로 화면에 갔다. 이제 네트워크 fetcher(`fetch_bulk_daily_prices*`, `fetch_toss_prices`)가 `PYTEST_CURRENT_TEST`에서 예외를 던진다(가드는 오케스트레이터가 아니라 **네트워크 가장자리**에 둔다 — 래퍼에 두면 내부를 스텁한 정상 테스트가 깨진다). provider 지연 자체는 결함이 아니라 last-good fallback이 설계된 이유다.

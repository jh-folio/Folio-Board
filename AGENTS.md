...
- **히트맵 라벨은 그린 뒤 타일을 재서 정한다**(0.5.5). 크기를 시가총액 비율(`7 + 21·√(cap/max)`)로만 정하던 시절에는 타일 픽셀을 몰라 넘치는 라벨이 생겼고, 그러면 Plotly가 통째로 축소해 1~5px 얼룩으로 남았다 — 실측 US 데스크톱 820×900에서 그려진 라벨 424개 중 **276개가 6px 미만**(중앙값 3px)이었다. "너무 작으면 비운다"는 규칙은 명목 크기로만 걸러 한 번도 걸리지 않았다. 빈 라벨로 한 번 그린 뒤 픽셀을 재고 들어가는 것만 얹으며, 캔버스 추정이 SVG와 어긋나면 축소 배율만큼 폭 예산을 깎아 최대 4회 수렴시키고 끝내 안 맞으면 포기한다. **줄여서 얹지 않는다.**
...
- **근거 검색은 질의별로 돌고 RRF(k=60)로 합친다**(0.5.5). 검색어를 공백으로 이어 붙이면 FTS5가 토큰을 OR로 풀어 어느 한 토큰만 스친 문서가 상위로 온다 — 실측으로 금리 리포트 근거에 인도 중앙은행·터키 물가·프랑스 강관회사 의결권 공시가 실렸다. **보도자료(`source_type=press_release`)는 근거에서 뺀다**; 규칙을 아는 곳은 `common/research_library/search/filters.py` 하나이고 브리핑의 `is_news_document()`도 그것을 부른다.
...
- **계획의 `requiredMacroData`를 실제로 조회한다**(0.5.5). 예전에는 계획에 적어 화면에 보여주고서 custom 고정값 3종만 받아왔다 — 기대인플레이션(`T10YIE`)·실질금리(`DFII10`)를 계획이 요청했는데도 보고서는 "장기금리 상승을 분해할 자료가 없다"고 적었다. 요청 시리즈는 LLM 자유 텍스트이므로 `FRED_SERIES_META` 허용 목록으로 거른다. FRED 관측치는 일간 기준 약 7년을 받아 분기 요약으로 싣고, 전년 대비는 인덱스가 아니라 **날짜**로 찾는다(`obs[12]`는 일간 시리즈에서 12영업일이다). 가격도 긴 구간(기본 10년)을 한 번 받아 분기 종가로 싣되 통계·상관관계 창은 `history_period` 그대로 둔다.
- **관련도 상위 근거에는 저장된 기사 본문을 함께 싣는다**(0.5.5). 실측으로 근거 29건 중 19건이 전문 보유였고 저장 본문 88,792자 중 모델에게 간 것은 스니펫 11,600자, 가진 자료의 13%였다. 상한은 `TOPIC_EVIDENCE_BODY_*` 환경변수가 정한다.
- **근거가 0건인 분석 축이 있으면 품질 점수에 상한을 건다**(0.5.5). 가중 평균만으로는 축 3개가 빈 보고서가 87점 A- pass를 받는다 — 축 절반 이상이 비면 59점, 하나라도 비면 74점이며 이유는 `quality.coverageCeiling`에 남는다. 축 하나가 통째로 비는 것은 감점이 아니라 결격이다.
...
- **거시 지표의 변화는 단위가 갈린다**(0.5.5). 금리처럼 이미 퍼센트인 계열은 차이가 `%p`지만, 지수·고용자 수·잔액 같은 수준값은 **비율(%)**로 내야 한다. 절대 차이를 단위 없이 표에 실었더니 모델이 근원 CPI 지수의 1년 차이 8.11포인트를 "전년 대비 +8.11%"로 읽어 보고서 본문에 실었다(실제 상승률은 약 2.5%). `macro_data.change_unit()`이 계열별 단위를 소유하고 표가 그 단위를 함께 찍는다.
- **보고서 골격은 머리·꼬리 고정 + 본문 자유다**(0.5.5). 예전에는 11개 섹션 전체가 고정이고 헤딩이 하나만 달라도 보고서를 통째로 버렸다. 유형별 템플릿 9개는 전부 같은 골격 안에서 강조점만 바꿨고, 계획이 다른 구성을 선언할 통로도 없었다(승인 계약이 정확 일치를 요구했다) — 그래서 주제와 맞지 않는 섹션은 건너뛸 수 없어 얇게 채워졌고(실측: 결론이 예산의 24%), 분석축 5개가 가중치 15%짜리 섹션 하나에 밀려 축당 400자를 받았다. **머리·꼬리는 서식이 아니라 계약이다** — 꼬리의 반론·시나리오·체크포인트·Source Notes는 §5 원칙 3(확증편향 방지)의 집행 장치이고, 체크포인트 추출·품질 평가·리더가 이름으로 찾는다. 가운데는 계획의 분석축이 그대로 섹션이 된다(`topic_schema.compose_sections`). 검증은 **필수 요소 누락만 차단**하고 본문 구성 차이는 결함(major)으로 남긴다 — id 하나, 제목 하나로 수 분짜리 산출물을 버리지 않는다. 분량도 섹션 목록에서 나온다(머리·꼬리 비중 고정, 남은 몫을 본문이 나눈다).
- **하위 질문은 검색용이 아니라 답해야 할 것이다**(0.5.5). 예전에는 질문 12개를 만들어 검색에만 쓰고 버렸다 — 생성 컨텍스트가 질문 목록을 넣으면서 "커버리지와 남은 갭을 Source & Data Notes에 반영하라"고만 지시했다. `axis_analysis.build_axis_briefs()`가 축마다 한 번씩 모델을 불러 그 축의 질문에 답하는 브리프(발견·수치·반대 근거·확인 못 한 것)를 만들고, 본문 생성이 그것을 뼈대로 쓴다. **축이 곧 본문 섹션이므로 브리프 하나가 섹션 하나다.** 브리프는 근거 안에서만 쓰며 모델이 지어낸 sourceId는 그 축이 실제로 본 근거와 대조해 버린다. 한 축이 실패해도 보고서를 죽이지 않는다(`status`로 남긴다) — 축 하나 때문에 수 분짜리 실행을 버리지 않는다.
- **본문은 개념 → 작동 원리 → 지금 어떤가 → 그래서 이 질문에는 순서로 쓴다**(0.5.5). 독자는 그 주제를 처음 보는 개인 투자자다(§0). 용어에 괄호 설명을 다는 것으로 그치지 않고 **무엇이 무엇을 움직이는지 단계로** 풀고, 표를 쓰면 표 아래에 읽는 법을 붙인다. 축 브리프도 `concept`·`mechanism`을 함께 받아 본문이 쓸 재료를 갖는다. 프롬프트가 12,000~16,000자를 요구하면서 같은 문서에서 "각 섹션을 간결하게라도 완성하는 것이 우선"이라고 적어 두면 모델은 뒤를 따른다 — 실측으로 모든 섹션이 예산의 20~60%만 찼다. 섹션 예산은 **하한**이며(최소 70%), 채울 말이 없으면 분량을 줄이지 말고 무엇을 확인하지 못했는지를 그 자리에 쓴다.
- **사용자 질문이 계약이다**(0.5.5). 40자로 줄인 주제어가 제목·검색을 지배하는 동안 원문 질문은 컨텍스트 한 줄로 밀려 있었고, 보고서가 질문의 절반(정책이 겪는 어려움)과 사용자가 직접 든 사례 하나를 통째로 빠뜨렸다(실측: 본문에 `2021` 0회, `2024` 0회, `제약`·`어려움` 0회). 이제 원문 질문이 생성 컨텍스트 최상단에 "반드시 답해야 할 질문"으로 들어가고 축별 분석 호출에도 함께 간다 — 축만 주면 모델이 그 축을 독립 주제로 답한다. **질문이 나열한 사례·구간은 분석축이 된다**(`planner.ensure_question_axes`) — 축이 곧 본문 섹션이라 축에서 빠지면 보고서에서 사라진다. 이미 덮인 항목은 중복 추가하지 않는다.
...
- **유형 템플릿은 섹션 이름을 부르지 않는다**(0.5.5). 골격을 풀고도 템플릿 9개가 `"5. 작동 경로"`, `"6. 수혜/피해 자산과 기업"`을 지시하고 있었고, `compose_prompt()`가 이걸 기본 프롬프트 **뒤에** 붙여 더 구체적·나중 지시로 읽혔다 — 모델이 계획의 축 5개를 버리고 옛 골격의 섹션 7개를 되살렸다. 지침은 **역할**로만 말한다.
...
- **핵심 논지를 본문 앞에서 고른다**(`thesis.py`, 0.5.6). 축별 브리프가 다섯 개 모여도 본문 생성은 그것을 병렬로 늘어놓았다 — 실측으로 인플레 → 금리 → 엔캐리 → 정책 → 한국 시장을 차례로 다루고 끝났고, 중심이 없으니 꼬리 섹션 넷이 같은 말을 되풀이했다. 축 분석 뒤·생성 앞에 한 번 멈춰 지금 근거가 가장 강하게 지지하는 결론 **하나**를 고른다. 가능성을 모두 적는 것은 판단이 아니라 판단의 회피이므로 나열을 금지한다. `confidence`는 enum(`high|medium|low`)이며 문장 강도로 옮겨 준다(§5 원칙 4). 논지 컨텍스트는 축 블록 **뒤**에 둔다 — 나중 지시가 더 구체적인 것으로 읽히므로 재료보다 프레임이 뒤에 와야 이긴다(템플릿 순서 버그에서 확인). 확증편향의 입구가 되지 않도록 **버린 해석과 반증 조건을 함께 싣고**, 반증 조건은 체크포인트 섹션의 뼈대가 된다(§5 원칙 3). 실패하면 빈 dict를 돌려주고 본문은 예전처럼 축 브리프로 쓴다.
- **축 브리프는 경쟁 가설과 반증 조건을 함께 받는다**(0.5.6). `counterEvidence`가 "어긋나는 자료"라면 `competingExplanations`는 "같은 자료의 다른 이야기"이고, `whatWouldChangeThis`는 "틀렸다면 무엇이 관측되어야 하는가"다. 새 단계를 만들지 않고 기존 브리프에 필드를 더한다 — 단계가 늘수록 CLI 비용과 실패 지점이 함께 는다.
- **못 쓸 초안은 쓰기만 한 번 더 시킨다**(`draft_guard.py`, 0.5.6). 딥 실행에서 값비싼 것은 쓰기가 아니라 그 앞이다 — 근거 팩·웹 조회(최대 4회)·축 브리프(최대 8회)·논지 선정(1회). 그런데 초안 생성은 한 번뿐이고 재시도 경로가 없어서 마지막 쓰기 한 번이 어긋나면 앞의 모든 것이 함께 버려졌다(실측 같은 질문 4회 중 2회가 못 쓸 초안이었고, 그중 하나는 필수 섹션 누락으로 **9분을 쓰고 아무것도 남기지 못했다**). 판정은 **차단 사유와 명백한 스텁만** 본다 — 고정 머리·꼬리 섹션 누락, 그리고 하한의 60%(`MIN_DRAFT_RATIO`) 미만. 문체·근거 연결은 보수 패스의 몫이라 재시도 사유에 넣지 않는다(여기서 걸러 버리면 보수가 할 일을 재생성이 대신한다). **재시도는 한 번이고**, 실패한 초안을 되돌려 주지 않으며(앵커가 되어 같은 실수를 되풀이한다) 무엇이 잘못됐는지만 짚는다. 재시도가 더 나쁘면 처음 것을 쓴다 — 나쁜 초안이라도 없는 것보다 낫다. 결과는 `draftGuard`에 남는다.
- **분석과 서술을 나눈다**(`editor.py`, 0.5.6). 최종 생성 호출이 분석가이면서 작가이면 모델은 안전한 쪽으로 기운다 — 실측으로 유보 표현이 천자당 3.07회(그중 `수 있다` 하나가 23회), 꼬리 섹션 넷이 같은 메시지를 반복, 웹에서 찾은 파월·월러 발언이 전부 "연준은 ~라고 설명했다"로 익명화됐다. 찾기/쓰기 분리와 같은 처방이다. 에디터는 결론 우선·caveat 압축·반복 통합·발언 실명 복원·문장 길이 변화만 하고 **새 사실을 만들 수 없다.** 그 금지는 프롬프트가 아니라 **코드가 집행한다**(`check_edit`) — 새 수치·새 근거 ID·근거 태그 유실·반론 축소·섹션 변경·분량 급감(85% 미만)·**분량 급증(125% 초과)**을 원문과 대조해 하나라도 걸리면 편집본을 버리고 초안을 쓴다. 표기 변형(7.0% → 7%)은 통과시킨다. 반론 태그는 **원문에서** 센다 — `visible_markdown`은 주석을 지우므로 그 위에서 세면 언제나 0이라 검사가 걸리지 않는다. 에디터 호출은 웹 검색을 끈다(사실을 더할 통로를 만들지 않는다). **늘리는 것도 막아야 한다** — 실측으로 초안 5,569자가 편집본 13,415자로 나왔고(+141%) 새 수치·새 ID가 없어 검사를 전부 통과했지만, 근거 없는 산문이 채워져 `unlinked_section` 7건과 `low_source_linkage`가 남았다. 초안이 짧은 것은 보수 패스가 다룰 일이지 에디터가 대신 쓸 일이 아니다.
- **에디터에게 분량을 맡기지 않는다.** 섹션 예산을 주면 하한까지 채우려 하고, 그러면 확장 상한에 걸려 통째로 버려진다(실측: 10,751자 초안에 16,086자(+50%)를 내놓아 거부). 짧은 초안은 보수 패스가 채운다.
- **문체 계약은 측정할 수 있는 둘만 잰다**(0.5.6). 유보 표현 밀도(`HEDGE_DENSITY_LIMIT` 천자당 2.5)와 한 표현 쏠림(10회 이상이면서 천자당 1.5 초과), 그리고 발언 귀속이다. Source & Data Notes는 데이터 한계 서술이 그 섹션의 일이므로 밀도에서 뺀다. 발언 귀속은 **이름이 아니라 직함으로** 본다 — 원문이 "Jerome H. Powell"이고 본문이 "파월"이라 글자가 겹치지 않지만 `의장`·`총재`·`이사`는 두 표기에 함께 남는다. 인용하지 않은 발언까지 귀속을 요구하지 않는다. "자연스러움"처럼 잴 수 없는 것은 계약에 넣지 않는다 — 규칙으로 강제한 서술 형식이 오히려 품질을 해친 실패를 두 번 겪었다.
...
- **품질 게이트가 계약 결함을 읽는다**(`apply_contract_ceiling`, 0.5.6). 결함 14건인 보고서가 93점/A/pass를 받았다 — 근거 없는 섹션 7개·분량 미달·본문 구성 불일치를 전부 안고서. 계약은 제대로 잡았는데 점수가 그것을 읽지 않아 사용자에게는 A로 보였다. 심각도 70 이상이면 69점, 40 이상이면 89점, 주요 결함 3건 이상이면 79점으로 누른다(실측 그 보고서는 69/C+/warn). 축 결측 상한과 같은 처방이며, 계약 결함은 딥 파이프라인만 아는 것이라 평가기를 일반적으로 두고 여기서 씌운다.

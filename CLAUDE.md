...
- **할인율은 회사별 WACC다.** 무위험수익률 + 조정베타×위험프리미엄, 세후 부채비용, 시가 기준 가중. **베타는 블룸 조정한다**(`2/3×원시 + 1/3×1`) — 원시 베타 2.12를 CAPM에 그대로 넣으면 할인율 14.8%를 받아 어떤 성장률로도 현재가가 설명되지 않는다. 모델이 답을 못 내는 것이지 그 회사가 그만큼 위험한 것이 아니다. 입력이 없으면 예전 고정값(9%)으로 내려가되 **그 사실을 보고서가 밝힌다**. 무위험수익률과 위험프리미엄은 가정이며 통화별 상수다(`riskFreeSource`가 밝히고, 살아 있는 값을 주입할 수 있게 인자로 열려 있되 여기서 조회하지 않는다).
...
- **계획 문서와 STATUS의 상태는 함께 바꾼다.** 계획 상단에는 `GitHub Issue`, `Status`, `Target release`, `Owner`, `Branch`를 두고, STATUS와 서로 다른 상태를 남기지 않는다. 한 문서 안의 단계가 서로 다른 릴리즈를 목표로 하면 계획을 분리하거나 대상 릴리즈를 복수 단계로 명시한다. 활성 계획은 현재 `plan/` 경로를 사용하며 옛 `roadmap/` 경로를 새 링크나 현재 위치 설명으로 복사하지 않는다.
...
- **진행 중인 작업은 실제 worktree 범위를 덮어야 한다.** 관련 tracked/untracked 변경을 확인해 STATUS의 작업명·범위와 대조하고, 별개의 목적이면 행이나 계획을 분리한다. 실질적 작업이 계획 없이 진행 중이면 그 사실만 기록하고 끝내지 말고, 릴리즈 포함 판단 전에 계획과 GitHub Issue를 만들거나 명시적으로 tiny one-turn 예외라고 판정한다.
- **GitHub Issue와 로컬 계획은 STATUS에서 교차 확인한다.** 열린 Issue의 대응 계획·대상 릴리즈·현재 상태를 표시하고, 완료된 작업의 열린 Issue, 서로 다른 제목/마일스톤, Issue 없는 실질적 계획, 계획 없는 열린 Issue를 방치하지 않는다. 공개-safe한 작업의 기준 기록은 GitHub Issue이고, STATUS는 그 관계와 현재 실행 상태를 보여주는 로컬 입구다.

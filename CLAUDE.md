...
- **주도 기업 선정 기준은 시장별 프롬프트가 소유하고 `PROMPT_REQUIRED_RULES`가 지킨다**(0.5.4). 4시장 분리 때 기준이 legacy `prompt.md`에만 남아 시장별 프롬프트에서 사라졌고, 기준 없는 모델이 보도량 상위의 니치 기업(Nebius·CoreWeave)과 SK hynix ADR을 미국장 주도 기업으로 올렸다. 절충은 자리 고정이 아니라 **점수**다(2026-08-22 사용자 결정) — `prioritize_briefing_groups(market_scope=...)`가 이야기 점수(보도량·적합도 합)에 시총 상위 구성종목 배율(`MAJOR_LEADER_MULTIPLIER=1.5`)을 곱한 `leaderScore`로 후보를 정렬하고, 프롬프트는 시장 영향력·이야기 두 축의 종합 점수로 두 기업을 고르라고 지시한다. 곱이라 이야기가 충분히 큰 니치는 여전히 이기고, 이야기 없는 조용한 대형주는 올라가지 않는다. 이야기 점수는 0 하한(`briefing_doc_score`)이라 곱이 안전하다 — 하한이 사라지면 음수×배율이 대형주를 거꾸로 끌어내리므로 절대값 가중으로 바꿔야 한다(테스트가 지킨다). 컨텍스트는 순서가 종합 점수임을 명시하고 가중 근거(`시장 시총 상위`)를 줄에 붙인다. 티커 연결은 이름이 아니라 문서 회사 태그의 ticker로 한다.

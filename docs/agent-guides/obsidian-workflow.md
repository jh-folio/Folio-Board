...
- Folio Board가 내보내는 1차 보고서/내러티브는 `generated_by: Folio Board`, `source_layer: primary_processed`, `reuse_as_evidence: false`를 가진다. 과거 `generated_by: Folio OS`로 내보낸 노트도 validator·importer가 똑같이 자기참조로 인식한다.

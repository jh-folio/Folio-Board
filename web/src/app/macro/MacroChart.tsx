...
/**
 * 거시 지표 한 개의 추이. 과거 시점 상세에서는 `comparison`(현재 수정치)을 점선으로 겹친다.
 * 테마·키보드·데이터 표·빈 상태는 FolioChart가 소유하므로 여기서 다시 구현하지 않는다.
 */
export function MacroChart({ title, points, comparison = [], stale = false }: {
  title: string;
  points: MacroPoint[];
  comparison?: MacroPoint[];
  stale?: boolean;
}) {
  ...
  const option = useMemo<OptionBuilder>(() => (tokens) => ({
    ...
    tooltip: {
      trigger: "axis",
      renderMode: "richText",
      valueFormatter: (value: unknown) => typeof value === "number" ? numberText(value) : "자료 없음",
    },
    ...
    series: [
      {
        ...
        showSymbol: false,
        connectNulls: false,
        data: points.map((p) => p.displayValue),
        itemStyle: { color: tokens("--folio-chart-1") },
      },
      ...(comparison.length ? [{
        name: "현재 수정치",
        type: "line",
  ...
  return (
    <FolioChart
      ...
      table={{
        title: `${title} 데이터 표`,
        unit: "관측기간",
        columns: ["관측기간", "표시값", "단위", ...(comparison.length ? ["현재 수정치"] : [])],
        rows: points.map((p) => [
          p.period,
          numberText(p.displayValue),
          p.displayUnit,
          ...(comparison.length ? [numberText(compared.get(p.period))] : []),
        ]),
      }}
      keyboard={{
        count: points.length,

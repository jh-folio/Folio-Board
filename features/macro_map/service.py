...
def observation_end(period, frequency):
    # 월·분기 관측은 그 기간의 마지막 날을 기준으로 노후를 잰다.
...
def map_snapshot(data_root: Path, *, market='US', mode='latest_revised', date=None, series_id=None, period=None, years=5, now=None):
    ...
    if selected > today:
        raise ValueError('future_macro_cutoff')
    if mode == 'latest_revised':
        selected = today
    # as_of는 선택한 날의 현지 마감까지 알려진 값만 읽는다.
    cutoff = day_end(selected.isoformat(), timezone) if mode == 'as_of' else None
    if period:
        dt.date.fromisoformat(period)
        if not series_id:
            raise ValueError('revision_series_required')
    if series_id:
        specs = [s for s in specs if s['id'] == series_id]
        if not specs:
            raise ValueError('unknown_macro_indicator')
    ...
    for spec in specs:
        ...
        # 품질 표시는 여러 개가 함께 붙을 수 있다. 하나의 성공/실패로 압축하지 않는다.
        quality = []
        if latest is None or latest['displayValue'] is None:
            quality.append('missing')
        if any(s.get('status') == 'provider_failed' for s in states):
            quality.append('provider_failed')
        if any(s.get('status') == 'partial' or s.get('cursor', {}).get('phase') in {'fred', 'fred_metadata', 'ecos'} for s in states):
            quality.append('partial')
        if latest:
            if (selected - observation_end(latest['period'], spec['frequency'])).days > spec['max_age_days']:
                quality.append('stale')
            if latest.get('revised'):
                quality.append('revised')
            if latest.get('conflict'):
                quality.append('vintage_conflict')
            if latest.get('calculationGap') == 'method_changed':
                quality.append('method_changed')

        # 방향은 표시 정밀도(소수 2자리) 기준 직전 관측 대비다. 좋고 나쁨을 뜻하지 않는다.
        ...
        # 과거 시점 상세에서만 현재 수정치를 겹쳐 그린다. 선택 시점 계산에는 쓰지 않는다.
        latest_compare = []
        ...
        items.append({
            ...
            'direction': direction,
            'quality': quality,
            'coverage': {'firstAvailableAt': first, 'sources': coverages},
            'providerStates': states,
            'releaseDateStatus': 'unknown',
            'nextRelease': schedules.get(spec['id']),
            'comparisonStatus': 'no_comparison_source',

...
- 빈 본문, 안전하지 않은 출처, 허용 목록 밖 source ID/section, 취소·기한 초과는 저장하지 않고 기존 저장본을 보존한다.

방향 오류 평가는 `validate_briefing_candidate()`를 통한 명시적 offline 평가에서만 수행한다. production 저장 경로는 고정 등락률로 본문을 교정하거나 방향 판정으로 거절하지 않는다.

production `finalValidation`/`validationRun`은 `assessmentStatus`와 `contentAssessment`를 `not_assessed`로 기록하며 `contradictionCount`·verified count를 0 또는 성공 수치로 기록하지 않는다. offline 의미 평가의 상세 결과는 명시적 평가 응답에만 남긴다.
...
- Agent 출력 계약을 어겨도 전체 보고서를 자동 재생성하지 않습니다(`retryOnViolation: 0`). 저장 단계는 authored Markdown을 보존하면서 형식·출처 참조·취소·기한 초과만 확인합니다. 명시적 품질 보완은 별도 `quality_repair` 작업으로 분리합니다.
...
### 히트맵 라벨과 색 (0.5.5, 0.6에서 ECharts로 이전)

**렌더러는 ECharts(SVG)입니다.** `public/vendor/echarts.js`(`window.echarts`)가 그리고, 이 파일(`public/briefing-visuals.js`)은 React 밖의 일반 스크립트라 `FolioChart`를 쓰지 않고 같은 라이브러리를 직접 부릅니다. 예전에는 Plotly로 그린 뒤 SVG 안의 `g.slice`·`__data__`·`_fullLayout`을 읽어 라벨을 얹고, Plotly가 넘치는 라벨을 통째로 축소하면 폭 예산을 깎아 4패스까지 다시 맞췄습니다(비동기 대기 포함). 그 103줄이 없어졌습니다.

**라벨은 그리기 전에 정확한 타일 크기로 정합니다.** squarify는 자료와 크기의 순수 함수라, DOM 없는 ssr 인스턴스에 같은 시리즈 설정으로 배치만 시키면 진짜 차트와 같은 좌표가 나옵니다(`heatmapTileSizes`). 그 폭·높이로 이름+등락률 → 어절 줄바꿈+등락률 → 이름만 순으로 들어가는 것만 고르고(`heatmapLabelPlan`), 안 들어가면 비웁니다 — **줄이거나 잘라 얹지 않습니다.** ECharts는 라벨을 축소하지 않고 넘치면 `...`로 자릅니다. 등락률이 잘리면 `-0.53%`가 `-0.5`로 읽혀 **틀린 숫자**가 되고, 이름이 잘리면 `Samsung…`이 12개사를 가리키므로, 처음부터 들어가는 것만 고르는 것이 이 방식의 핵심입니다.

**배치를 읽는 경로는 ECharts의 공개 API가 아닙니다**(`getModel().getSeriesByIndex(0).getData().tree`의 `getLayout()`). 그래서 벤더 파일을 정확한 버전으로 고정하고, `public/briefing-visuals.test.js`의 canary가 실제 벤더 번들로 이 경로를 매번 확인하며(ECharts를 올리면 여기서 먼저 걸립니다), 읽지 못하면 `null`을 돌려주어 **보수적인 추정**(실측 타일 폭 ÷ √면적의 최소 0.58)으로 물러납니다 — 라벨은 줄지만 잘리지는 않습니다. ssr 인스턴스는 `dispose`하지 않으면 Node 프로세스가 끝나지 않습니다(6.1.0).

**좌우 여백은 칸 폭의 8%(최소 3px)씩이고, 트리맵 라벨의 기본 padding은 0으로 둡니다.** 잘림을 막는 쪽과 보기 좋은 쪽이 따로 있습니다.

- **잘림**: ECharts 트리맵 라벨의 기본 `padding`은 5라서 글자에 쓸 수 있는 폭이 칸 폭보다 양쪽 5px씩 줄어듭니다 — 계획한 폭이 넉넉히 들어가는데도 잘렸습니다(브라우저 실측: 여백 5px 이하에서 잘림 다수). padding을 0으로 두면 KR·JP·US 모두 여백 1px부터 잘림 0이었고 0px에서만 잘렸습니다. 기본 `▶ ` 접두(`drillDownIcon`)도 폭을 먹으므로 끕니다.
- **미관**: 여백을 3px로 고정하면 폭에 꼭 맞게 들어가는 라벨이 칸 가장자리에 붙어 보입니다(실측: 라벨 폭이 칸 폭의 85%를 넘는 타일이 KR 8개·JP 14개). 그래서 여백을 **칸 폭의 8%**로 두고 좁은 칸에서만 3px를 하한으로 씁니다(최소 3px는 글꼴이 늦게 내려와 폭이 조금 달라져도 받아낼 여유입니다). 그러면 라벨은 칸 폭의 84%를 넘지 않고, 대가는 라벨 몇 개뿐입니다(KR 69→67, JP 137→135, US 268→265). 위아래는 3px씩입니다 — 라벨이 세로 가운데라 양쪽이 같고, 세로로 칸의 85%를 넘는 라벨은 없었습니다.

글꼴을 바꾸면 여백과 함께 다시 잽니다.

실측(820×900, 종목 라벨을 받은 타일 수, 잘림 0): KR 2026-08-27 **67**개(예전 Plotly 63), JP 2026-09-16 135개, US 2026-09-11 265개(1200×900은 329개). 좁은 화면(520px 미만, 375×620)은 섹터만 그려 11개입니다. 글자 하한은 6px이고 라벨은 어느 시장에서도 칸 폭의 84%를 넘지 않습니다.

**글자 크기는 칸 크기를 따라갑니다.** "들어가기만 하면 최대"로 두면 **글자 길이가 크기를 정합니다** — 작은 칸이라도 티커가 짧으면(`MU`) 큰 칸과 같은 크기를 받아 칸에 비해 글자가 큽니다. 그래서 칸마다 `6 + 0.105·√면적`(상한 20px)으로 쓸 수 있는 최대 크기를 먼저 정하고, 그 안에서 들어가는 가장 큰 크기를 고릅니다. 완전 비례가 아니라 면적의 제곱근을 따라가는 이유는, 면적을 그대로 쓰면 큰 칸만 남고 나머지가 전부 하한으로 눌리기 때문입니다. 줄 간격은 글꼴 기본값에 맡기지 않고 `글자 크기 × 1.3`으로 직접 지정하며(`rich` 스타일), 세로로 들어가는지도 그 값으로 잽니다.

**섹터·산업 머리띠와 깊이 상한에서 잎이 된 그룹(좁은 화면의 `Materials`)도 같은 계획기를 씁니다.** 안 그러면 ECharts 기본 처리가 `▶ M...`처럼 자릅니다. 머리띠는 26px 높이에 한 줄이고 글자는 13px까지입니다. 이름이 안 들어가는 좁은 머리띠는 띠는 남기고 글자만 비웁니다(`show: false`로 끄면 띠 자체가 사라져 자식이 그 자리를 먹고, 빈 문자열은 서식 없음으로 읽혀 기본 이름이 잘려 그려지므로 공백 한 글자로 "글자 없음"을 명시합니다). 보이지 않는 뿌리는 머리띠를 갖지 않게 합니다(맨 위 26px이 빈 띠로 남지 않도록). 머리띠 색은 fill이 아니라 자식 사이 틈의 색(`borderColor`)으로 칠해집니다.

**첫 화면에서 산업 층을 접습니다.** 산업 127개가 각각 머리띠와 여백을 가져가면서 종목이 쓸 자리를 먹는데, 정작 그 머리띠는 실측에서 **13%(127개 중 16개)만** 읽혔습니다. 첫 화면은 섹터→종목이고, 섹터를 누르면 그 섹터의 산업이 새 뿌리가 되어 열립니다(넓은 화면은 종목까지 한 겹 더). 산업을 누르면 그 산업의 종목만 남습니다. 라이브러리의 드릴다운(`nodeClick`)은 쓰지 않고 **층 노드의 자식을 새 뿌리로 넘겨 다시 그립니다.** 누를 때마다 더 자세해지는 방향이며 되돌아오는 길은 히트맵 위 경로(`전체 › Technology › Semiconductors`)가 맡습니다. 이름 띠는 경로가 이미 보여 주므로 다시 그리지 않습니다. **종목 타일을 눌러도 아무 일이 없습니다**(마지막 층이다).

**머리띠 클릭은 좌표로 가립니다.** ECharts는 머리띠를 눌러도 그 섹터가 아니라 보이지 않는 뿌리를 알려 줍니다. 배치 선계산이 준 좌표로 머리띠 위인지 직접 판정합니다(`heatmapHeaderAt`). 정확한 크기를 못 읽어 추정으로 물러난 경우에는 좌표를 몰라 머리띠 클릭이 꺼지고, 좁은 화면(섹터가 타일)의 클릭은 그대로 됩니다.

**폭이 바뀌면 다시 물어 다시 그립니다.** `ResizeObserver`가 폭을 알리면 타일 크기를 다시 묻고 라벨을 다시 계획합니다(깊이도 폭이 정합니다 — 520px 미만은 섹터만). 읽을 비동기 레이아웃이 없으므로 예전처럼 창을 넓혔더니 라벨이 167개에서 78개로 줄던 경합이 없고, 같은 크기를 두 번 그리면 같은 결과입니다.
...
- ECharts(SVG, `public/vendor/echarts.js`): 섹터→산업→종목 계층의 전체 시장 treemap을 그린다. KR처럼 산업명이 섹터명과 같은 경우는 중복 단계를 생략해 섹터→종목으로 표시한다. 면적은 시가총액, 색은 일간 등락률이다. viewport에 가까워질 때 lazy render하고 export 직전에는 렌더를 보장한다.
...
## 문체 평가의 경계 (style_check.py)

`style_check.py`는 명시적 offline 평가에서만 사용할 수 있다. production 저장 경로는
`styleCheck`를 생성하지 않으며, 문체 결과 때문에 authored Markdown을 되돌리거나 저장을
막지 않는다. Agent의 일반 생성도 semantic/style/fact-repair/model 호출 없이 저장한다.

...
**좌우 여백은 3px이고, 트리맵 라벨의 기본 padding을 0으로 둡니다.** ECharts 트리맵 라벨의 기본 `padding`은 5라서 글자에 쓸 수 있는 폭이 칸 폭보다 양쪽 5px씩 줄어듭니다 — 계획한 폭이 넉넉히 들어가는데도 잘렸습니다(브라우저 실측: 여백 5px 이하에서 잘림 다수). padding을 0으로 두면 KR·JP·US 모두 여백 1px부터 잘림 0이었고 0px에서만 잘렸습니다. 3px는 예전 Plotly 때와 같은 숨 쉴 자리이자 글꼴이 늦게 내려와 폭이 조금 달라져도 받아낼 여유입니다. 글꼴을 바꾸면 다시 잽니다. 기본 `▶ ` 접두(`drillDownIcon`)도 폭을 먹으므로 끕니다.

실측(820×900, 종목 라벨을 받은 타일 수, 잘림 0): KR 2026-08-27 **69**개(예전 Plotly 63), JP 2026-09-16 137개, US 2026-09-11 268개. 375×620 US는 87개입니다. 글자 하한은 6px입니다.

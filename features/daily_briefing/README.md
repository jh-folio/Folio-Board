...
**섹터가 한눈에 갈라져 읽히게 합니다 — 등락 색과 경계를 따로 둡니다.** 예전에는 섹터 이름이 13px 흰 글자였고, 섹터 경계가 종목 경계와 같은 얇은 흰 선이라 띠가 옆 종목 칸에 섞였습니다(IT 띠는 바로 아래 초록 칸과 거의 같았습니다). 지금은:

- **머리띠는 섹터 평균 등락 색 그대로입니다**(시가총액 가중, `heatmapColor` 9단). 색이 바로 "이 섹터가 오늘 어땠는가"의 답이라 남깁니다. 그 위 흰 글자는 9단 모두에서 대비 AA(4.5:1) 이상입니다(테스트가 등락 색 전부로 확인합니다). 평균 등락은 색뿐 아니라 **글자로도** 띠에 적습니다(`Financials  -0.71%`).
- **섹터 경계는 등락 색과 별개인 무채색 바깥선입니다**(`heatmapSectorOutlines`). 트리맵은 머리띠·틀·틈을 모두 한 색(`borderColor`)으로 칠해서, 띠를 섹터 색으로 두면 테두리를 따로 줄 수 없습니다. 그래서 배치 선계산이 준 섹터 사각형 위에 `graphic` 사각형(채움 없음, 2px 선)을 얹습니다. 선 색은 잉크 토큰(72%)이라 **라이트에선 어둡고 다크에선 밝아** 섹터 색과도 카드 배경과도 갈리며, 테마가 바뀌면 다시 그립니다. `silent`라서 머리띠 클릭과 툴팁을 막지 않고, 선은 사각형 안쪽으로 그려 이웃 섹터 쪽 틈을 침범하지 않습니다. 정확한 크기를 못 읽어 추정으로 물러난 경우에는 좌표를 몰라 바깥선이 없습니다.
- **섹터 사이 틈은 4px, 종목 사이 틈은 1px**입니다. 섹터 틈에는 카드 배경이 비쳐 나오고, 종목 칸은 테두리 없이 1px 틈만 둡니다(틈은 부모의 색으로 비칩니다). 좁은 화면에서 섹터가 타일이 될 때도 같은 바깥선이 섹터 타일을 가릅니다.
...
- 머리띠 글자 폭은 ECharts가 **좌우 padding 6px과 틀 두께 1.5px씩**을 뺀 값으로 재므로 `heatmapHeaderPlan`도 같은 값과 잘림 여유 2px을 뺍니다. 안 그러면 `Heavy Industr...`처럼 잘렸습니다.

...
**섹터가 한눈에 갈라져 읽히게 합니다.** 예전에는 섹터 이름이 13px 흰 글자였고 띠 색이 **섹터 평균 등락 색**이라 옆 종목 칸과 같은 색 계열이 되었습니다(IT 띠는 바로 아래 초록 칸과 거의 같았습니다). 섹터 경계도 종목 경계와 같은 얇은 흰 선이라 어디서 섹터가 시작하는지 알기 어려웠습니다. 지금은:

- **머리띠는 등락 색이 아니라 무채색 슬레이트(`#3b4251`)입니다.** 색은 종목 칸이 말하고 띠는 "여기서 섹터가 시작된다"만 말합니다. 흰 글자와의 대비는 약 10:1(AA 4.5:1의 두 배 이상)이고 라이트·다크 카드 배경 양쪽에서 띠가 보입니다. 섹터의 **평균 등락은 색이 아니라 글자로** 띠에 적습니다(`Financials  -0.71%`, 시가총액 가중).
...
- **섹터 사이 틈은 4px, 종목 사이 틈은 1px**입니다. 섹터 틈에는 카드 배경이 비쳐 나오고 섹터는 1.5px 슬레이트 틀로 둘러싸입니다. 종목 칸은 테두리 없이 1px 틈만 둡니다(틈은 부모의 borderColor, 곧 슬레이트로 비칩니다). 좁은 화면에서 섹터가 타일이 될 때도 같은 틀이 섹터 타일을 가릅니다.

...
생성 API는 `markets` 또는 호환용 `marketScope`로 미국·한국·유럽·일본 중 하나 이상을 받습니다. 각 시장은 `0~6 + 오늘의 결론` 골격과 `한 줄 결론 + 가운뎃점 3~4개 + 줄글` 순서를 유지합니다. 주도 기업은 기본적으로 두 자리지만, KR 일일 concentration control의 활성 모드에서는 직접 근거를 충족한 기업만 0~2자리로 둡니다. 저장 JSON의 시장별 section에는 markdown·세션·기준일·출처와 생성·근거 추적 메타데이터가 따로 남고, 기존 소비자는 결합 `markdown`을 계속 읽을 수 있습니다.
...
- Agent 출력 계약을 어겨도 전체 보고서를 자동 재생성하지 않습니다(`retryOnViolation: 0`). writeback 전에 실패하므로 이전 정상 보고서는 유지되고, 허용되는 자동 보강은 근거 범위 안의 한정된 섹션 교체뿐입니다.

...
- **이름은 14px 굵게**(등락은 한 걸음 작고 옅게)이고 11px 밑으로 내리지 않습니다. 한 줄에 안 들어가는 좁은 섹터는 두 줄(`Energy &` / `Chemicals`, 9~10px)로 나누고, 그래도 안 되면 비웁니다 — 통째로 비면 어느 섹터인지 알 수 없어서 두 줄을 먼저 시도합니다. 이름이 안 들어가는 아주 좁은 띠는 글자를 비우되 hover 툴팁이 이름과 평균 등락을 말합니다. 띠 높이는 28px입니다.

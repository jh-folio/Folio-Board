...
**섹터는 갈라져 읽히되 지도는 한 장으로 읽힙니다 — 경계는 간격의 위계가 말합니다.** 두 가지를 함께 잡아야 합니다. 예전에는 섹터 경계가 종목 경계와 같은 얇은 선이라 섹터가 안 갈렸고, 그 반대로 섹터마다 굵고 진한 바깥선을 두르면 지도가 한 장이 아니라 카드 여러 개로 보였습니다. 지금은:

- **모든 틈과 틀이 같은 색(투명 — 카드 배경이 비칩니다)이고 굵기만 다릅니다.** 종목 사이 1px, 섹터 사이 3px + 양쪽 틀 1px(= 5px). 라이트에선 흰 선, 다크에선 어두운 선으로 지도 전체가 한 가지 선으로 나뉜 한 장이 됩니다. 굵은 무채색 **바깥선은 기본으로 끕니다**(`HEATMAP_SECTOR_OUTLINE_ALPHA = 0`, 켜면 `heatmapSectorOutlines`가 잉크색 선을 섹터마다 얹습니다).
- **머리띠는 섹터 평균 등락 색 그대로입니다**(시가총액 가중, `heatmapColor` 9단). 색이 바로 "이 섹터가 오늘 어땠는가"의 답이라 남깁니다. 트리맵은 띠·틀·틈을 `borderColor` 한 색으로 칠하므로 띠 색은 **라벨 배경**으로 따로 칠하고 `borderColor`는 투명으로 둡니다 — 그래야 띠는 섹터 색이고 틈은 카드 색일 수 있습니다. 그 위 흰 글자는 9단 모두에서 대비 AA(4.5:1) 이상입니다(테스트가 등락 색 전부로 확인합니다). 평균 등락은 색뿐 아니라 **글자로도** 띠에 적습니다(`Financials  -0.71%`).
- **머리띠와 그 아래 종목 칸은 1px 구분선으로 갈립니다**(`heatmapHeaderRules`). 띠가 섹터 색 그대로라 아래 칸과 맞붙으면 한 덩어리로 보이는데, 종목 사이 틈과 **같은 색·같은 굵기**의 선을 띠 아래에 두면 지도가 한 가지 선으로 나뉜 채 띠만 칸과 갈립니다. 카드 배경색은 그리는 시점에 읽고 테마가 바뀌면 다시 그립니다. `silent`라서 머리띠 클릭과 툴팁을 막지 않습니다.
- **이름은 14px 굵게**(등락은 한 걸음 작고 옅게)이고 11px 밑으로 내리지 않습니다. 한 줄에 안 들어가는 좁은 섹터는 두 줄(`Energy &` / `Chemicals`, 9~10px)로 나누고, 그래도 안 되면 비웁니다 — 통째로 비면 어느 섹터인지 알 수 없어서 두 줄을 먼저 시도합니다. 이름이 안 들어가는 아주 좁은 띠는 글자를 비우되 hover 툴팁이 이름과 평균 등락을 말합니다. 띠 높이는 28px이고 **글자는 띠의 세로 가운데**에 앉습니다 — 줄 높이가 글자 크기와 같으면 ECharts가 글자를 띠 맨 위에 붙여 그리고(글자 중심이 띠 중심보다 7px 위, 브라우저 실측) `verticalAlign: "middle"`은 효과가 없어서, 한 줄은 줄 높이를 띠 높이(28px)로, 두 줄은 14px씩으로 둡니다(두 줄이 정확히 띠를 채웁니다). 이름과 등락이 같은 줄 높이를 써야 같은 세로 위치에 앉습니다.
- 머리띠 글자 폭은 ECharts가 **좌우 padding 6px과 틀 두께 1px씩**을 뺀 값으로 재므로 `heatmapHeaderPlan`도 같은 값과 잘림 여유 2px을 뺍니다. 안 그러면 `Heavy Industr...`처럼 잘렸습니다.
섹터 머리띠와 깊이 상한에서 잎이 된 그룹(좁은 화면의 `Materials`)은 종목과 다른 계획기를 씁니다(`heatmapHeaderPlan`, 잎은 `heatmapLabelPlan`). 안 그러면 ECharts 기본 처리가 `▶ M...`처럼 자릅니다. 안 들어가는 머리띠는 띠는 남기고 글자만 비웁니다(`show: false`로 끄면 띠 자체가 사라져 자식이 그 자리를 먹고, 빈 문자열은 서식 없음으로 읽혀 기본 이름이 잘려 그려지므로 공백 한 글자로 "글자 없음"을 명시합니다). 보이지 않는 뿌리는 머리띠를 갖지 않게 합니다(맨 위 28px이 빈 띠로 남지 않도록).

**마우스를 올려도 기업명이 그대로입니다.** 올린 칸은 ECharts가 별도의 **강조(emphasis) 상태**로 그리는데, 그 상태의 라벨은 시리즈 기본(`show: false`)을 따라 통째로 꺼져 기업명이 사라졌습니다. 라벨을 가진 칸은 강조 상태에서도 켜고(서식은 평상시 것을 물려받습니다), 라벨이 없는 칸은 켜지 않습니다 — 켜면 올렸을 때 기본 이름이 좁은 칸에 튀어나옵니다. 종목 칸에는 강조 색·테두리를 더하지 않습니다(차트 마크는 정지 상태만 있다는 디자인 결정).

**글꼴이 다 내려온 뒤 한 번 더 그립니다.** 라벨 폭은 캔버스로 재는데 웹 글꼴(Inter·IBM Plex·SUIT)이 아직 안 내려왔으면 대체 글꼴로 잽니다. 다 내려온 뒤 ECharts가 진짜 글꼴로 그리면 글자가 조금 넓어져 계획한 폭에 딱 맞던 라벨이 `...`로 잘립니다. 그리는 시점에 글꼴이 아직 로딩 중이면 `document.fonts.ready` 뒤에 다시 재서 계획을 새로 세웁니다(이미 다 내려와 있으면 하지 않습니다).
...
**일봉 종가가 빈 종목의 보완.** 야후는 한 세션의 일봉을 시가·거래량만 채우고 종가를 몇 시간씩 비워 두는 일이 있고, 다음 세션이 생기면 그 일봉을 아예 빼 버립니다(2026-09-22 세션: S&P 500 500개 중 485개). 같은 시세 응답에는 그 세션 종가가 이미 있으므로, 재시도 뒤에도 빈 종목만 야후 일괄 시세(`v7/finance/quote`, 100개씩)로 한 번 더 채웁니다(`market_universe._fill_empty_daily_closes`). 날짜가 분명할 때만 씁니다 — 시세가 **대상 세션 장 마감 후**의 것이면 그 가격을, **시장 달력상 바로 다음 거래일**의 것이면 그 전일 종가를 씁니다. 사이에 다른 세션이 끼었거나 달력을 모르는 시장이면 채우지 않고 누락으로 둡니다. 전일 종가는 달력의 직전 거래일 일봉 종가입니다. 채운 종목이 있으면 경고에 `N symbols used the provider's regular-market close ...`로 남습니다. 실측 대조: 일봉 종가가 있던 45개 중 44개가 일치했고 LRCX 하나가 0.106% 달랐습니다(정규장 마감가와 일봉 종가의 기록 차이).

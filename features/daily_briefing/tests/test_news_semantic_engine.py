...
def test_scope_gates_do_not_create_transport():
    ...
    assert transport.make_news_semantic_engine(market="us", kind="weekly", engine="cli") is None
    assert transport.make_news_semantic_engine(market="jp", engine="cli") is None
    assert transport.make_news_semantic_engine(market="kr", selected_markets=["us"], engine="cli") is None
...
def test_injected_callback_receives_core_payload_unchanged_and_is_one_call():
    ...
    engine = transport.make_news_semantic_engine(market="us", engine="cli", llm_call=fake)
    ...
    assert seen[0][1]["max_output_tokens"] == transport.SEMANTIC_OUTPUT_BUDGET
...
def test_size_limits_are_utf8_and_checked_before_and_after_call():
    ...
    engine = transport.make_news_semantic_engine(
        market="kr", engine="cli", llm_call=lambda *_args, **_kwargs: calls.append(True) or {},
    )
    assert engine is not None
...
def test_budget_is_checked_before_and_after_transport():
    ...
    budget = Budget()
    engine = transport.make_news_semantic_engine(
        market="us", engine="cli", budget=budget,
...
def test_retired_api_transport_is_unavailable(monkeypatch):
    monkeypatch.delenv("PYTEST_CURRENT_TEST", raising=False)
    assert transport.make_news_semantic_engine(market="us", engine="api") is None

...
- 찾는 것: 최근 분기의 **필수 묶음**(`FACT_TOPICS`) — 전사 실적(회계·조정 구분),
  사업부별 실적, 시장별 성장률·비중, 가이던스와 변경 폭, 자본배분(배당 변경·자사주
  금액과 평균가·설비투자 계획) — 과 **경영진의 실제 발언**(이름과 직함). SEC 제출
  실적발표(Exhibit 99.1)·회사 IR 원문을 요약 기사보다 우선한다. 주가 전망이나 투자
  의견은 찾지 않는다 — 근거가 아니라 남의 판단이다.
- **묶음마다 상한을 둔다**(묶음당 3, 사실 전체 14, 발언 4, 사실 한 줄 400자). 묶음 표시가 없거나 알 수 없는 사실은 전체 상한만 받고(`other`), 표시가 하나도 없으면 "찾지 못한 묶음"을 적지 않는다. 전체 8건
  상한이던 때는 같은 원문을 두 번 조회해도 매번 다른 묶음이 빠졌다(2026-09 HWM 실측:
  한 번은 사업부·가이던스, 한 번은 시장별 성장·예비부품·자사주, 배당 인상은 두 번 다
  누락). 찾기 호출은 여전히 1회다. 블록은 묶음별로 모아 보여 주고 **찾지 못한 묶음을
  명시**해, 본문이 조회 누락을 "회사가 공시하지 않았다"로 바꿔 쓰지 않게 한다.
...
- `scope_undefined`(40) — `핵심 판단` 안에 다루는 것과 **다루지 않는 것**을 밝히고, 그 문단을
  `분석 범위:`로 시작한다. 검사 낱말은 평가기와 같은 목록이고 넓히지 않는다(평가기를 쓰는
  다른 보고서의 점수가 함께 바뀐다). 예전에는 두 스타일 프롬프트도, 요구 블록의 예시 문장도
  그 낱말을 쓰지 않아 범위를 제대로 밝힌 보고서가 감점됐다(2026-09 Opus 저장본).
- 체크포인트 표의 칸도 방향만이 아니라 조건으로 쓴다("한 자릿수로 둔화" → "한 자릿수가
  되면"). 방향만 적은 표는 결함으로 둔다(`test_scenario_and_scope_checks.py`).
...
## 입력의 기간·정의 정합성 (2026-09 HWM 후속)

- **순차입금은 한 기준일의 잔액이다**(`sec_companyfacts.debt_position`). 같은 날짜에 차입금 조합과 현금이 함께 있는 가장 최근 날짜를 고른다. 정의가 넓은 순서: `DebtLongtermAndShorttermCombinedAmount` → 비유동 장기부채+`DebtCurrent` → … → `LongTermDebt`(유동분 포함이라 `LongTermDebtCurrent`와 더하지 않는다). 그 날짜에 단기차입 보고가 없으면 0으로 채우지 않고 `complete=false`로 표시한다. **완전한 조합을 모든 날짜에서 먼저 찾고**, 불완전한 조합은 그다음이다 — 최신 분기가 장기부채만 보고했다고 단기차입까지 있는 연말 조합을 덮으면 차입금이 줄어든다. 불완전한 기준일 값은 연차 행에 단기차입이 있으면 쓰지 않는다. 연차 경로에서 확인되지 않은 항목은 `missing`으로 남겨 표시하는 쪽이 "0으로 계산"을 밝힌다. 현금은 제한성 현금을 뺀 값을 먼저 쓴다. `dcf.net_debt_from`이 이 값을 연차 잔액보다 늦거나 같을 때 쓰고, 기준일·정의를 `asOf`·`basis`로 함께 싣는다(HWM: 2025년 말 $2.31B → 2026-06-30 $3.94B). DCF 컨텍스트·규칙 보고서의 순부채 행·재무 안정성 근거가 같은 값을 읽는다.
- **흐름 지표는 최신 회계연도 값만 지금의 판단에 쓴다**(`financial_engine.current_year_value`). 매출·순이익·영업현금흐름의 최신 연도보다 오래된 이자비용·배당은 역사로 남기고 비율·판정의 입력으로 쓰지 않는다. 차입비용(`debtCost`)은 같은 해의 이자와 연말 부채로 나눈다. 재무 품질 표는 오래된 배당 금액 대신 "최신 회계연도 값 미확인(마지막 공시 연도)"를 적는다.
- 후보 태그: 보통주 배당의 현재 태그 `PaymentsOfOrdinaryDividends`를 더했고, 한 번도 잡히지 않던 오타 `InterestExpenseNonOperating`을 실제 태그 `InterestExpenseNonoperating`으로 고쳤으며, 총이자 계열 `InterestAndDebtExpense`를 더했다. 순이자(`…NonoperatingNet`)는 이자수익을 뺀 다른 정의라 섞지 않는다.
- **표시만 반올림한다.** DCF·자사주 컨텍스트의 큰 금액은 `format_value`로 B/M 단위로 보여 준다. 계산 객체의 정밀도는 그대로다.
- **주가 수익률도 본문 입력이다**(`service.render_price_return_context`). 화면 차트와 같은 1·3·6·12개월 수익률과 마지막 종가일(`asOf`)을 싣는다. 가격에 담긴 기대의 변화라는 맥락으로만 쓰며 매매 시점·가격 수준별 행동은 쓰지 않는다. 차트 값이 없으면 블록을 생략한다.
- 이전 저장본에는 소급하지 않는다.
...
- 링크 이름: 웹 조회 항목(`web_reference`)은 원장 제목이 사실 문장이라 `출처 — 짧은 요지`(요지 60자)로 쓰고, 그 밖의 긴 제목은 120자 안에서 단어 경계에 줄임표를 붙여 자른다. 문장 중간에서 끊긴 발언 링크가 있었다(2026-09 HWM).

...
def project_events(events):
    ...
    for event in events:
        ...
        if re.fullmatch(r'\d{6}01',period):period=period[:6]
        unit=event.get('unit')
        try:value=Decimal(str(event.get('actualValue') or ''))
        ...
        if key not in seen:
            copy={**event,'additionalSources':[]};seen[key]=copy;output.append(copy)
        else:

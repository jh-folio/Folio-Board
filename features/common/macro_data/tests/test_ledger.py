...
def point(period='2024-01-01', value='100', vintage='2024-02-01', *, series='CPIAUCSL', unit='Index', fetched='2026-09-27T01:00:00+00:00', **extra):
    return {
        ...
        'fetchedAt': fetched,
        'metadata': {'unit': unit, 'frequency': 'M', 'adjustment': 'SA', 'definitionVersion': 'v1'},
        **extra,
    }


def write(store, *rows):
    return store.ingest(rows[0]['seriesId'], rows, cursor={'page': 2}, status='ok')
...
def test_quarterly_staleness_counts_from_end_of_observed_quarter(tmp_path):
    ...
    write(
        store,
        point('2026-01-01', series='GDPC1', vintage='2026-07-01', metadata={'unit': 'Dollars', 'frequency': 'Q', 'adjustment': 'SA', 'definitionVersion': 'v1'}),
...
def test_local_observation_is_not_backdated_to_measurement_period(tmp_path):
    ...
    with pytest.raises(ValueError, match='korean_historical'):
        map_snapshot(tmp_path, market='KR', mode='as_of', date='2024-02-01')
...
def test_page_validation_is_atomic_and_preserves_cursor(tmp_path):
    ...
    with pytest.raises(ValueError):
        store.ingest('CPIAUCSL', [point(value='102', vintage='2024-04-01'), point(value='nan')], cursor={'page': 3})
...
def test_database_failure_rolls_back_rows_and_cursor(tmp_path):
    ...
    with pytest.raises(sqlite3.IntegrityError):
        store.ingest('CPIAUCSL', [point(value='102', vintage='2024-04-01')], cursor={'page': 3})
    assert len(store.revisions('CPIAUCSL', '2024-01-01')) == 1
    assert store.state('CPIAUCSL')['cursor'] == {'page': 2}


@pytest.mark.parametrize('scenario,base,current', [
    ('normal', '100', '102'),
    ('inflation_shock', '100', '115'),
    ('tightening', '110', '113'),
    ('recession_transition', '110', '105'),
])

...
def test_cancel_does_not_advance_a_page(tmp_path):
    class Reader:
        def fred_pages(self, *args, **kwargs):
            yield [point()], {'phase': 'fred', 'chunk': 100}, '2026-09-27T01:00:00+00:00'
    ...
    with pytest.raises(RuntimeError):
        collect(tmp_path, reader=Reader(), selected={'CPIAUCSL'}, cancel=cancel)
...
def test_monthly_fred_pages_preserve_metadata_and_pagination():
    ...
    def fetch(endpoint, params, required):
        ...
        if endpoint == 'series':
            return {'seriess': [
                {'id': 'CPIAUCSL', 'units': 'Old index', 'frequency_short': 'M', 'seasonal_adjustment_short': 'SA', 'realtime_start': '2000-01-01', 'realtime_end': '2024-02-01'},
                {'id': 'CPIAUCSL', 'units': 'New index', 'frequency_short': 'M', 'seasonal_adjustment_short': 'SA', 'realtime_start': '2024-02-02', 'realtime_end': '2026-09-26'},
            ]}, '2026-09-27T01:00:00Z'
        if endpoint.endswith('vintagedates'):
            return {'count': 2, 'vintage_dates': ['2024-02-01', '2024-02-02']}, '2026-09-27T01:00:00Z'
        if params.get('output_type') == 1:
            return {'count': 1, 'observations': [{'date': '2024-01-01', 'value': '200', 'realtime_start': '2024-02-02'}]}, '2026-09-27T01:00:00Z'
...
def test_scheduler_off_is_read_only_and_missed_runs_coalesce(tmp_path, monkeypatch):
    ...
    def submit(*args, **kwargs):
        ...
        return {'id': f'test-{len(submissions)}', 'status': 'queued'}
...
def test_shared_job_contract_supports_macro_without_ai_or_artifact():
    ...
    job = new_shared_job(
        kind=JobKind.MACRO_REFRESH,
        task_type=TaskType.MACRO_REFRESH,

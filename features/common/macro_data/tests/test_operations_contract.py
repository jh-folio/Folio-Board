...
def test_schema_ddl_failure_rolls_back_and_existing_data_is_backed_up(tmp_path,monkeypatch):
    ...
    with sqlite3.connect(path) as c:c.execute('CREATE TABLE notes(id TEXT)');c.execute("INSERT INTO notes VALUES('keep')")
    monkeypatch.setattr(store_module,'DDL',store_module.DDL+('INVALID SQL',))
    with pytest.raises(sqlite3.OperationalError):MacroStore(path).ensure()
...
def test_429_retries_and_timeout_does_not_expose_url(monkeypatch):
    ...
    def request(*a,**k):
        calls.append(1)
        if len(calls)<3:raise urllib.error.HTTPError('https://example.test?api_key=secret',429,'limited',{},None)
    ...
    def timeout(*a,**k):raise TimeoutError('https://example.test?api_key=secret')
    monkeypatch.setattr(providers.urllib.request,'urlopen',timeout)
    with pytest.raises(providers.ProviderError) as caught:providers.get_json('never-output')
...
def test_stale_cache_is_not_accepted_as_a_successful_fetch():
    ...
    with pytest.raises(providers.ProviderError):reader.fred('series',{},'seriess')


def test_daily_long_form_resumes_offset_and_keeps_withdrawal():
    reader=providers.OfficialReader(None,fred_key='test');requests=[]
    def fetch(endpoint,params,required):
        requests.append(params)
        if endpoint=='series':return {'seriess':[{'id':'DFF','units':'Percent','frequency_short':'D','seasonal_adjustment_short':'NSA','realtime_start':'2000-01-01','realtime_end':'2026-09-26'}]},'2026-09-27T00:00:00Z'
        if endpoint.endswith('vintagedates'):return {'count':1,'vintage_dates':['2024-01-02']},'2026-09-27T00:00:00Z'
...
def test_ecos_refuses_wrong_or_extra_dimension():
    ...
    reader._fetch=lambda *a,**k:({'StatisticSearch':{'list_total_count':1,'row':[{'STAT_CODE':'901Y009','ITEM_CODE1':'0','ITEM_CODE2':'different','TIME':'202408','UNIT_NAME':'Index','DATA_VALUE':'1'}]}},'2026-09-27T00:00:00Z')
    with pytest.raises(providers.ProviderError,match='dimension'):list(reader.ecos_pages(BY_ID['KR_CPI'],'2000-01-01'))
...
def test_scheduling_deduplicates_active_job_across_slot_and_write_failure(tmp_path,monkeypatch):
    ...
    with pytest.raises(OSError):operations.submit_refresh(tmp_path)
...
def test_calendar_day_link_cannot_set_official_release_or_leak_future_schedule(tmp_path):
    ...
    upsert_events(store.path,[{'kind':'macro','provider':'fred','title':'미국 CPI','market':'US','status':'confirmed','startsAt':'2024-03-01T08:30:00-05:00','sourceUrl':'https://fred.stlouisfed.org/release?rid=10','fetchedAt':'2024-02-02T12:00:00Z'}])
...
def test_real_shared_job_lifecycle_uses_macro_result_and_cancel_boundary(tmp_path,monkeypatch,outcome):
    ...
    jobs.shared_store().add(job);jobs.private_lifecycle().set_private(job.id,{})
    def worker(root,*,start,cancel):
        if outcome=='cancelled':jobs.cancel_job(job.id);cancel()
    ...
    if outcome=='done':assert saved['result']['savedCount']==2


def test_restart_terminates_orphan_and_keeps_committed_page(tmp_path,monkeypatch):
    from features.common import jobs
    monkeypatch.setattr(jobs,'JOBS_PATH',tmp_path/'jobs.json');jobs._LIFECYCLES.clear()
    job=jobs.new_shared_job(kind='macro_refresh',task_type='macro_refresh',generation_mode='none',adapter='none',requested_mode=None,mode='collect',attempted_engine=None,clock=jobs._clock)
    jobs.shared_store().add(job);jobs.private_lifecycle().set_private(job.id,{})
    store=MacroStore(tmp_path/'market-memory.sqlite3');write(store,point())
...
def test_cache_budget_preserves_fresh_and_unowned_files(tmp_path):
    ...
    for path in (old,fresh,unrelated):path.write_text('x'*80)
    os.utime(old,(0,0));os.utime(fresh,(10000,10000));os.utime(unrelated,(0,0))
...
def test_unreadable_cache_cleanup_does_not_fail_a_collection(tmp_path,monkeypatch):
    ...
    from features.common.macro_data.cache_policy import prune_owned_cache
    def denied(_):raise PermissionError('locked cache directory')

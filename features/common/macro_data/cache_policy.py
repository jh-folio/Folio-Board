...
def prune_owned_cache(root: Path, *, budget_bytes=128 * 1024 * 1024, now=None):
    ...
    try:
        ...
        return 0
    for path in paths:
        # 이 모듈이 만든 거시 응답 cache 파일만 지운다. 링크·하위 폴더·다른 파일은 건드리지 않는다.
        if not _OWNED.fullmatch(path.name) or path.is_symlink() or not path.is_file() or path.resolve().parent != root:
            continue
        try:
            stat = path.stat()
        except OSError:
    ...
    for modified, size, path in sorted(candidates):
        ...
        if clock - modified <= 3600:
            continue
        if total <= budget_bytes and clock - modified <= 7 * 86400:
            continue
        try:
            path.unlink()
        except OSError:

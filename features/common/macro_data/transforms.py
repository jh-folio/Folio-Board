...
def compatible(a, b):
    # 단위·주기·계절조정·정의가 하나라도 다르면 두 값을 비교하지 않는다.
    return all(a.get(k) == b.get(k) for k in ('unit', 'frequency', 'adjustment', 'definitionVersion'))


def transform(points, spec):
    # points는 같은 cutoff로 고른 이력이다. 기준기간 값도 그 cutoff의 값이므로
    # 나중에 개정된 기준값이 과거 시점의 전년비에 섞이지 않는다.
    ...
    for p in points:
        ...
        if kind in {'yoy', 'mom', 'qoq', 'difference'}:
            value = None
            if raw is None or base is None or base['value'] is None:
                reason = 'comparison_missing'
            elif not compatible(p['metadata'], base['metadata']):
...
def spread(corporate, government):
    # 같은 관측일의 두 금리 차. 한쪽이라도 없으면 계산하지 않는다.
    corp = {p['period']: p for p in corporate}
    gov = {p['period']: p for p in government}
    result = []
    for period in sorted(corp.keys() | gov.keys()):
        left = corp.get(period)
        right = gov.get(period)
        present = [p for p in (left, right) if p]
        p = present[0]
        ...
        result.append({
            **p,
            ...
            'metadata': metadata,
            'metadataId': digest(metadata),
            'displayValue': value,
            'displayUnit': '%p',
            'transform': 'spread',
            'calculationGap': None if valid else 'comparison_missing',
            'inputs': [left, right],
            'revised': any(v.get('revised') for v in present),
            'conflict': any(v.get('conflict') for v in present),

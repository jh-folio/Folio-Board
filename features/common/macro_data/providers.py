...
def get_json(url: str):
    for attempt in range(3):
        try:
            with urllib.request.urlopen(url,timeout=40) as response:return json.load(response)
...
def parse_fred_vintages(payload: dict,series_id: str):
    ...
    if not isinstance(rows,list):raise ProviderError('invalid_fred_payload')
    pattern=fred_vintage_pattern(series_id)
    for point in rows:
        if not isinstance(point,dict):raise ProviderError('invalid_fred_row')
...
def fred_metadata(row):
    return {'unit':row['units'],'frequency':row['frequency_short'],'adjustment':row['seasonal_adjustment_short'],
            'definitionVersion':digest([row.get('title'),row.get('notes')]),'providerSeries':row['id']}


class OfficialReader:
    def __init__(self,runtime:ProviderFetchRuntime,*,fred_key='',ecos_key='',transport=get_json):
        self.runtime=runtime;self.keys={'fred':fred_key,'ecos':ecos_key};self.transport=transport
        self.metadata_versions={}

    def _fetch(self,provider,operation,params,url,required):
        def retrieve():
            result=self.transport(url)
            if not isinstance(result,dict) or required not in result:raise ProviderError('invalid_provider_payload')
            return result
        result=self.runtime.fetch(provider,'macro_'+operation,{**params,'parserVersion':'macro-1'},retrieve,
            policy=FetchPolicy(ttl_seconds=3600,timeout_seconds=60,stale_while_revalidate_seconds=0),background_refresh=False)
        if result['status'] not in {'fresh','cached'}:raise ProviderError('provider_failed')
        return result['value'],result['fetchedAt']

    def fred(self,endpoint,params,required):
        if not self.keys['fred']:raise ProviderError('not_connected')
        url='https://api.stlouisfed.org/fred/'+endpoint+'?'+urllib.parse.urlencode({**params,'api_key':self.keys['fred'],'file_type':'json'})
        cache_params={**params}
        if endpoint=='series/observations':cache_params['metadataVersion']=self.metadata_versions.get(params['series_id'],'unknown')
    ...
    def fred_pages(self,spec:Series,start:str,*,cursor=None,cancel=lambda:None):
        ...
        if not isinstance(meta_rows,list) or not meta_rows:raise ProviderError('vintage_metadata_missing')
        for meta in meta_rows:
            if not isinstance(meta,dict) or meta.get('id')!=spec.code or meta.get('frequency_short')!=spec.frequency or not meta.get('units') or not meta.get('seasonal_adjustment_short'):
                raise ProviderError('invalid_fred_metadata')
            if dt.date.fromisoformat(meta['realtime_start'])>dt.date.fromisoformat(meta['realtime_end']):raise ProviderError('invalid_fred_metadata')
        ...
        while True:
            ...
            payload,_=self.fred('series/vintagedates',dict(series_id=spec.code,realtime_start=start,realtime_end=end,limit=1000,offset=offset),'vintage_dates')
            page=payload['vintage_dates']
            if not isinstance(page,list):raise ProviderError('invalid_vintage_dates')
            dates.extend(page);offset+=len(page)
            if offset>=int(payload.get('count',len(dates))):break
            if not page:raise ProviderError('incomplete_vintage_page')
        if any(not isinstance(d,str) for d in dates) or dates!=sorted(set(dates)):raise ProviderError('invalid_vintage_dates')
        if any(not start<=dt.date.fromisoformat(d).isoformat()<=end for d in dates):raise ProviderError('invalid_vintage_dates')
        if not dates:raise ProviderError('vintage_unavailable')
        first_vintage=dates[0]
        metadata_dates=sorted({m['realtime_start'] for m in meta_rows if max(start,since)<=m['realtime_start']<=end and m['realtime_start']>first_vintage})
        ...
        while chunk<len(dates):
            cancel();batch=dates[chunk:chunk+batch_size]
            ...
            for row in parse_fred_vintages(payload,spec.code):
                if row['vintageDate'] not in batch or row['period']<start:raise ProviderError('fred_range_mismatch')
                meta=next((m for m in meta_rows if m['realtime_start']<=row['vintageDate']<=m['realtime_end']),None)
                if meta is None:raise ProviderError('vintage_metadata_missing')
                points.append({**row,'seriesId':spec.id,'availabilityBasis':'provider_vintage','fetchedAt':fetched,'metadata':fred_metadata(meta)})
            size=len(payload['observations']);total=int(payload['count'])
            if total<0 or offset+size>total:raise ProviderError('invalid_observation_count')
            if not size and offset<total:raise ProviderError('incomplete_observation_page')
            offset+=size
            if offset>=total:chunk+=batch_size;offset=0
        ...
        index=int(cursor.get('metadataIndex',0));offset=int(cursor.get('offset',0)) if cursor.get('phase')=='fred_metadata' else 0
        while index<len(metadata_dates):
            cancel();day=metadata_dates[index]
            payload,fetched=self.fred('series/observations',dict(series_id=spec.code,observation_start=start,output_type=1,realtime_start=day,realtime_end=day,limit=10000,offset=offset),'observations')
            meta=next(m for m in meta_rows if m['realtime_start']<=day<=m['realtime_end'])
            points=[]
            for row in payload['observations']:
                if row['realtime_start']!=day or row['date']<start:raise ProviderError('fred_range_mismatch')
                points.append({'seriesId':spec.id,'period':row['date'],'value':row['value'],'vintageDate':day,'availabilityBasis':'provider_vintage','fetchedAt':fetched,'metadata':fred_metadata(meta)})
            size=len(payload['observations']);total=int(payload['count'])
            if offset+size>total or (not size and offset<total):raise ProviderError('incomplete_observation_page')
            offset+=size
            if offset>=total:index+=1;offset=0
    ...
    def _fred_interval_pages(self,spec,start,end,since,dates,meta_rows,cursor,cancel):
        ...
        if not dates:return
        window=cursor.get('window') or max(since,dates[0]);offset=int(cursor.get('offset',0))
        while window<=end:
            upper=min(end,f'{int(window[:4])+1}-12-31')
            boundaries=[m['realtime_start'] for m in meta_rows if window<m['realtime_start']<=upper]
            if boundaries:upper=(dt.date.fromisoformat(min(boundaries))-dt.timedelta(days=1)).isoformat()
            cancel()
            ...
            for r in payload['observations']:
                ...
                if not window<=vintage<=upper or period<start:raise ProviderError('fred_range_mismatch')
                meta=next((m for m in meta_rows if m['realtime_start']<=vintage<=m['realtime_end']),None)
                if meta is None:raise ProviderError('vintage_metadata_missing')
                points.append({'seriesId':spec.id,'period':period,'value':r['value'],'vintageDate':vintage,'availabilityBasis':'provider_vintage','fetchedAt':fetched,'metadata':fred_metadata(meta)})
            size=len(payload['observations']);total=int(payload['count'])
            if total<0 or offset+size>total:raise ProviderError('invalid_observation_count')
            if not size and offset<total:raise ProviderError('incomplete_observation_page')
            offset+=size
            if offset>=total:window=(dt.date.fromisoformat(upper)+dt.timedelta(days=1)).isoformat();offset=0
    ...
    def ecos_pages(self,spec:Series,start:str,*,cursor=None,cancel=lambda:None):
        if not self.keys['ecos']:raise ProviderError('not_connected')
        cursor=cursor or {};end=cursor.get('end') or dt.datetime.now(ZoneInfo(spec.timezone)).date().isoformat()
        ...
        while True:
            cancel()
            params={'series':spec.id,'table':spec.code,'frequency':spec.frequency,'items':spec.items,'start':start,'end':end,'offset':offset,'limit':1000,'metadataVersion':digest(spec.public())}
            ...
            body=payload['StatisticSearch'];page=body.get('row');total=int(body.get('list_total_count',0))
            if not isinstance(page,list) or not page:raise ProviderError('empty_ecos_page')
            if offset+len(page)-1>total:raise ProviderError('invalid_ecos_count')
            points=[]
            for r in page:
                if r.get('STAT_CODE')!=spec.code or any(str(r.get(f'ITEM_CODE{i+1}',''))!=item for i,item in enumerate(spec.items)):raise ProviderError('ecos_dimension_mismatch')
                if any(r.get(f'ITEM_CODE{i}') not in (None,'','?') for i in range(len(spec.items)+1,5)):raise ProviderError('ecos_dimension_mismatch')
                period=ecos_period(r['TIME'],spec.frequency)
                if not start<=period<=end:raise ProviderError('ecos_range_mismatch')
                meta={'unit':r['UNIT_NAME'],'frequency':spec.frequency,'adjustment':spec.adjustment,'providerSeries':spec.code,'dimensions':list(spec.items),'definitionVersion':'ecos-'+spec.code}

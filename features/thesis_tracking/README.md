...
- **신규 티커 정규화는 `model.normalize_ticker` 하나입니다**(노트 색인과 동일 규칙 — `.KS/.KQ` 제거·`.`→`-`). 기존 별칭 키는 이력과 소유권을 유지하며 조회·갱신합니다. 정확한 기존 키, 정규화 키, 유일한 기존 별칭 순서로 찾고, 여러 별칭이 모호하게 겹치면 임의로 선택하거나 병합하지 않습니다. Delta와 Watchlist 조회도 해석된 기존 저장 키를 사용합니다.

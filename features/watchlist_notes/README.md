...
- 재무·투자 지표는 `GET /api/market/fundamentals?symbol=`
  ...
  **배당수익률은 이미 % 단위다**(도요타 3.26 = 3.26%) — fraction으로 오해해 100을 곱하면
  ...
  사람이 PER과 PBR을 구분할 수 없다. 표 옆에는 분기 이익 차트(매출·영업이익·순이익,
  최근 5분기, 분기 손익계산서 기반)가 붙는다. 적자 분기는 기준선 아래로 그린다 — 0으로
  접으면 "이익 없음"으로 읽힌다. **캐시 키에 스키마 버전이 들어간다** — 없으면 필드를
  추가한 판올림 직후 옛 모양의 캐시가 내려와 새 화면이 조용히 비어 있다(실측).

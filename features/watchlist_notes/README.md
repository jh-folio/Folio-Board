...
상세는 **재무·투자 지표 → 네이티브 차트 → 실적 → 수집 뉴스** 순이다. 네 섹션이 같은
면(`surface--group`)과 같은 머리 문법(굵은 제목 좌 + 보조 컨트롤 우)을 쓰고, 데스크톱에서는
차트와 실적이 2컬럼으로 나란히 선다. 간격은 `.watchlist-detail-grid`의 gap 하나가 소유한다 —
예전에는 실적의 일부만 상자에 있고 차트·뉴스는 맨바닥인 데다 자식 margin이 gap에 더해져,
어떤 정보가 한 묶음인지 눈이 읽지 못했다.

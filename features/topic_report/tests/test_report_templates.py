...
def test_prompt_does_not_tell_the_model_to_be_brief():
    ...
    이 회귀를 막던 이전 장치는 "각 섹션 최소 70%"라는 고정 하한이었다. 하지만 Phase 1
    B0/B1 실측 비교(0.6, 2026-09-13)에서 이 하한을 포함한 엄격한 형식(B0)보다, 하한 없이
    "완결에 필요한 만큼, 채우기 위한 반복은 금지"로 쓴 자유 형식(B1)의 글이 사용자 판단으로
    더 낫다고 확인됐다 — 표만 다시 허용하는 조건으로 채택. 그래서 지금은 하한 숫자가 아니라
    "간결/브리핑 압박이 없다"와 "채우기용 반복 금지"라는 반대쪽 원칙 둘로 같은 회귀를 막는다.
    숫자 하한 자체의 부활 여부를 검사하지 않는다 — 하한이 없는 것이 이번 계약이다.
    """
    path = os.path.join(_ROOT, "features", "topic_report", "prompt.md")
    with open(path, encoding="utf-8") as fh:
        prompt = fh.read()
    ...
    assert "채우기 위한 분량은 아니다" in prompt, "완성 원칙이 있어야 한다(숫자 하한 대신)"
    assert "분량을 채우지 마라" in prompt, "채우기용 반복 금지 원칙이 있어야 한다"

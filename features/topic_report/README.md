...
## CLI 완료 상태와 안전한 재개 (2026-09-22)

- 승인된 작성·축 분석·웹 조회·편집·부분 보수는 공통 E0 결과를 읽습니다. 제공자가 미완료/실패로 끝난 결과는 글의 섹션이 갖춰져 있어도 정상 완료로 취급하지 않습니다. 작성 결과는 검증 후보로 넘기고, 보조 호출의 불완전 결과는 성공한 조사/편집으로 채택하지 않습니다.
- 불완전 작성 결과에는 추가 초안 재시도나 편집을 자동 실행하지 않습니다. 후보 검증, 공통 정상본 저장, 기동 시 후보 복구에서 차단합니다. 기존 정상 보고서와 Overlay는 유지합니다. 실패 후보를 별도 사용자 라이브러리로 영구 보관하는 기능은 아니며 후보 보관은 기존 job-context 수명을 따릅니다.
- 취소·시간 초과는 선택적 보조 단계에서도 전파하며, 다음 모델 호출/후보 기록/최종 결과 전 취소 상태를 확인합니다. 집필 웹 검색 설정은 조사 시 확정한 값을 사용합니다. 실제 CLI 이어쓰기 호출은 지원하지 않으며 기존의 단계 체크포인트 재사용과 구별합니다.
- 재개 지문은 계획·일자·선택 ID·모델/추론 정책 외에 실제 입력 자료(정규화 전후)·시장/매크로 입력·시장 맥락·웹 출처 정책도 포함합니다. 시장 자료 최상위 `asOf`는 조회 시각이므로 제외하지만 실제 관측 날짜와 가격은 유지합니다. 같은 ID의 내용이나 검색 정책이 바뀌면 다시 조사합니다. 실패 조회는 체크포인트에 남기지 않습니다. 구 지문은 불일치로 재사용하지 않으며 저장 보고서를 변경하지 않습니다.
- 관측 종료/usage/검색 상태의 safe projection만 보고서 `executionFacts`에 남습니다. private native citation은 실행 중 결과 객체에서 본문 편집/보수 뒤 보수적으로 재매핑하며 원본 응답·session/continuation ID는 보고서나 진단에 저장하지 않습니다. 인용의 화면/내보내기 표시는 이번 연결에 포함하지 않습니다. 미관측 결과와 과거 보고서는 unknown 호환을 유지합니다.
...
딥 리서치 보완 집필은 링크 문단을 뺀 원래 본문을 대상으로 검증한다. 선택한 후보마다 원래 E0 결과를 다시 매핑해 수정된 범위의 native 인용을 떨어뜨리며, 재시작용 accepted 후보에도 안전한 Markdown 링크를 저장한다. 실제 provider 세션 이어쓰기·추가 조사 실험은 이 인용 구현에 포함하지 않는다.

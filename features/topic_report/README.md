...
- 재개 지문은 계획·일자·선택 ID·모델/추론 정책 외에 실제 입력 자료(정규화 전후)·시장/매크로 입력·시장 맥락·웹 출처 정책도 포함합니다. 같은 ID의 내용이나 검색 정책이 바뀌면 다시 조사합니다. 실패 조회는 체크포인트에 남기지 않습니다. 구 지문은 불일치로 재사용하지 않으며 저장 보고서를 변경하지 않습니다.

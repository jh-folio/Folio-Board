...
```text
머리(고정) Executive Summary · 질문 정의와 분석 범위 · 핵심 데이터 대시보드
본문(계획)  분석축이 그대로 섹션 — 2~8개
꼬리(고정) 반론과 리스크 · 시나리오 · 앞으로 확인할 체크포인트 · 결론 · Source & Data Notes
```
...
- **분량도 섹션 목록에서 나온다**(`depth_policy`). 머리·꼬리 비중은 고정이고 남은 몫(딥 41%)을 본문 섹션이 나눈다. 축이 늘면 축당 분량은 줄지만 본문 전체 몫은 유지된다.
...
- **섹션 예산은 하한이다(최소 70%).** 예전 프롬프트는 12,000~16,000자를 요구하면서 같은 문서에서 "각 섹션을 간결하게라도 모두 완성하는 것이 우선"이라고 적어 뒀고, 모델은 뒤를 따랐다 — 실측으로 모든 섹션이 예산의 20~60%만 찼다. 채울 말이 없으면 분량을 줄이지 말고 무엇을 확인하지 못했는지와 그 한계를 그 자리에 쓴다. `test_report_templates.py`가 이 문구가 되살아나지 않는지 검사한다.
- 본문 섹션이 꼬리(반론·시나리오·체크포인트)와 겹치면 **본문은 분석을, 꼬리는 판단표를** 맡는다. 본문에서 "아래 표 참조"로 미루면 그 섹션이 비어 버린다(실측 262자).
